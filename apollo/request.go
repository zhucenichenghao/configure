...
func SendRequest(r *Request) (respBody []byte, err error) {
	...
	if err != nil {
		rlog.Error("create request failed", map[string]interface{}{
			"error": err.Error(),
		})
		return nil, err
	}
	...
	if err != nil {
		rlog.Error("request failed", map[string]interface{}{
			"error": err.Error(),
		})
		return nil, err
	}
	...
	if err != nil {
		rlog.Error("read response body failed", map[string]interface{}{
			"error": err.Error(),
		})
	}
	return respBody, err
